...
class World {
    constructor(size, scene) {
        ...
        for (let x = 0; x < this.dimension.x; x++) {
            for (let y = 0; y < this.dimension.y; y++) {
                for (let z = 0; z < this.dimension.z; z++) {
                    for (let nx = Math.max(0, x - 1); nx <= Math.min(x + 1, this.size - 1); nx++) {
                        for (let ny = Math.max(0, y - 1); ny <= Math.min(y + 1, this.size - 1); ny++) {
                            for (let nz = Math.max(0, z - 1); nz <= Math.min(z + 1, this.size - 1); nz++) {
                                if (nx != x || ny != y || nz != z) {
